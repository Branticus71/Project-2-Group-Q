...
```{r}
get_recipes <- function(cuisine ="N/A", diet = "N/A", time = 300, number = 100, allergies = "N/A", API = "ea88d9f6e2bc470bbddeb6db653f214b"){
  url <- paste0("https://api.spoonacular.com/recipes/complexSearch?apiKey=", API, "&maxReadyTime=", time, "&number=", number, "&sort=random&addRecipeNutrition=true")
  if(cuisine != "N/A") {
    url <- paste0(url,"&cuisine=", cuisine )
  }
  if(diet != "N/A") {
    url <- paste0(url, "&diet=", diet)
  }
...
get_wine <- function(type, price= 6000, number = 100, API = "05671e9eac144e52907ef1a7a7e37874"){
  ...
  wine_rec <- wine_list$recommendedWines %>% 
    ...
    mutate(type = type, averageRating = averageRating * 5)
  return(wine_rec)
}
...
wine_rec_df <-wine_list %>%
  lapply(get_wine) %>%
  bind_rows() %>% mutate(price = parse_number(price))
wine_rec_df
...
#Possible wrapper function for above
...
testing_wrap<- wrapper_recipes(cuisine_list, number = 5)
testing_wrap
